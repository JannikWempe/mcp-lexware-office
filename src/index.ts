...
import { makeLexOfficeRequest } from "./helper.js";
...
server.tool(
  ...
  "Get a list of invoices from LexOffice",
  {
    ...
    page: z
      ...
      .optional()
      .default(0)
      .describe("page number to retrieve; starts at 0"),
    size: z
      .number()
      .min(1)
      .max(250)
  ...
  async ({ status }) => {
    ...
    const voucherlistData = await makeLexOfficeRequest<any>(voucherlistUrl);
...
server.tool(
  ...
  async ({ id }) => {
    ...
    const invoiceData = await makeLexOfficeRequest<any>(invoiceUrl);
...
server.tool(
  ...
  async ({ email, name, number, customer, vendor }) => {
    ...
    const contactsData = await makeLexOfficeRequest<any>(contactsUrl);
...
async function main() {
  ...
  logger.log("LexOffice MCP Server running on stdio");
}
